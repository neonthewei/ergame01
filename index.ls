  ...
  ..controller \ERGame, <[$scope $interval $timeout $http $sce]> ++ ($scope, $interval, $timeout, $http, $sce) ->
    ...
    $scope.game = do
      ...
      countdown: do
        ...
        start: ->
          ...
          try
            ga 'send', 'event', 'ergame03', getToday!, 's'+(new Date).getTime!.toString!, 0
          @value = 5
          @count!
    $scope.doctor = do
    ...
    $scope.config = do
      ...
      mode: do
        ...
        easy: [
          * prob: {pat: [0.02, 0.60, 0.95], sup: 0.01, stay: 0.3}, decay: {life: 0.001, sup: 0.005, mad: 0.001}
          * prob: {pat: [0.04, 0.50, 0.82], sup: 0.02, stay: 0.3}, decay: {life: 0.002, sup: 0.008, mad: 0.002}
    ...
    interval = do
      spawn: ->
        ...
        if time <= 60 => $scope.config.cur = $scope.config.mode[$scope.mode]0
          ...
          $scope.doctor.faint = true
          $scope.doctor.demading = 0
        else if time <= 85 => $scope.config.cur = $scope.config.mode[$scope.mode]0
        else if time <= 86 => 
          it = $scope.percent.sprite.points.filter(->(it.type in [2 3 4]) and it.variant > 0)[0]
          it.mad = 1
          it.ismad = true
        else if time <= 99 => $scope.config.cur = $scope.config.mode[$scope.mode]1
        else if time <= 100 => 
          it = $scope.percent.sprite.points.filter(->(it.type in [2 3 4]) and it.variant > 0)[0]
          it.mad = 1
          it.ismad = true
        else if time <= 123 => $scope.config.cur = $scope.config.mode[$scope.mode]2
        else => $scope.config.cur = $scope.config.mode[$scope.mode]3
        $scope.debug.d2 = time
        $scope.debug.d3 = $scope.config.cur.prob.pat.2
        if time >= 108 and time <= 116 and $scope.game.state == 2 => $scope.danger = true
        else if time <= 123 => $scope.danger = false
        if isHalt! => return
        r = Math.random!
      ...
      drain: ->
        ...
        if madmax and !$scope.madmax =>
          ...
          $scope.madmax = $scope.madmax_prev == 1 && 2 || 1
          $scope.madmax_prev = $scope.madmax
          $scope.doctor.demading = 0
    ...
    $scope.share = do
      update-rank: ->
        ...
        obj = do
          ...
          redirect_uri: \http://0media.tw/p/ergame/
          description: "一款富含真實情境的經典急診室經營夢幻之作，為台灣第一個急診室新聞遊戲。遊戲背景鎖定在台灣的一間大型醫學中心，面對健保體制的崩壞、沒膽改革的政府以及愛跑大醫院看病的人民，擁有拯救急診室命運能力的鍵盤醫師，將在一次又一次的真實的醫療突發狀況中突圍，試圖拯救病患的生命。你，將在人類的極限體力、醫生的使命和病患的生命中作出抉擇，準備好了嗎？"
        @rank = ["#k=#{encodeURIComponent(v)}" for k,v of obj].join(\&)
      game: ["#k=#{encodeURIComponent(v)}" for k,v of args].join(\&)
...
window.ctrl = do
  ...
  tutorial: (is-touch = false, event) -> @wrap is-touch, ~>
    ...
    @scope!audio.click!
    try
      ga 'send', 'event', 'ergame03', getToday!, 't'+(new Date).getTime!.toString!, 0
...
window.touch = touch = do
  down: (e) ->
    ...
    if s.is-min or s.is-pad =>
      if (e.target and e.target.nodeName.toLowerCase! == "a")
      or (e.target.parentNode and e.target.parentNode.toLowerCase! == "a") => return
      e.prevent-default!
  up: (e) ->
    touchflag := true
