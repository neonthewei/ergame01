...
(function(){
  var getToday, ref$, w, h, patw, path, of1x, of1y, dpw, dph, x$, touchflag, audioinit, touch;
  getToday = function(){
  ...
  x$.controller('ERGame', ['$scope', '$interval', '$timeout', '$http', '$sce'].concat(function($scope, $interval, $timeout, $http, $sce){
    ...
    interval = {
      spawn: function(it){
        ...
        if (time <= 60) {
          ...
          it = $scope.percent.sprite.points.filter(function(it){
            var ref$;
        ...
        if (time >= 108 && time <= 116 && $scope.game.state === 2) {
          $scope.danger = true;
        } else if (time <= 123) {
          $scope.danger = false;
        }
    ...
    $scope.image = {
      ...
      init: function(){
        ...
        return $scope.assets.fetch('assets/img.gz', 'image/png', function(arg$){
  ...
  window.ctrl = {
    ...
    tutorial: function(isTouch, event){
      ...
      return this.wrap(isTouch, function(){
        ...
        this$.scope().audio.click();
        try {
          return ga('send', 'event', 'ergame03', getToday(), 't' + (new Date).getTime().toString(), 0);
        } catch (e$) {}
      });
    },
